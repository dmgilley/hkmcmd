...
import sys, warnings
import numpy as np
from hkmcmd import utility, interactions, voxels, reaction, system, io
...
def main(argv):
    ...
    while Reacting:
        ...
        reactive_events_list = reaction.get_reactive_events_list(
            molecules_list,
            diffusion_rates_dict_matrix,
            reaction_scaling_df,
    ...
    atoms_list, bonds_list, angles_list, dihedrals_list, impropers_list = (
        interactions.get_interactions_lists_from_molcules_list(molecules_list)
    )

    if args.debug is True:
        breakpoint()

    io.write_lammps_data(
        args.prefix + ".in.data",
        atoms_list,
        bonds_list=bonds_list,
        angles_list=angles_list,
    ...
    lammps_init = io.LammpsInitHandler(
        prefix=args.prefix,
...
def update_molecules_list_with_reaction_rsATmd(
    ...
    molecules_list = interactions.update_molecules_list_IDs(molecules_list, reset_atom_IDs=False)
    molecules_list = interactions.remove_overlaps_from_molecules_list(
