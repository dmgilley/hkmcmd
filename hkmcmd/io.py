            ...
            if frame_flag == 1:
                ...
                if atom_flag == 1:
                    ...
                    if atom_count == N_atoms:
                        ...
                        # Upwrap the geometry
                        if unwrap is True:
                            atoms_list = interactions.unwrap_atoms_list(
...
def parse_data_file(
    ...
    atoms_list = [interactions.Atom(ID=ID) for ID in atom_ids]
    ...
    if unwrap is True:
        atoms_list = interactions.unwrap_atoms_list(atoms_list, box)
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if species is not None:
                data[species].append([float(_) for _ in fields])
                continue
    species = sorted(list(data.keys()))
    return {sp: np.array(data[sp]) for sp in species}
