...
from hkmcmd import utility, interactions
...
def read_hkmcmd_json(filename):
    ...
    if data_dict["species"] is not None:
        data_dict["species"] = [interactions.Molecule(**item) for item in data_dict["species"]]
    if data_dict["reactions"] is not None:
        data_dict["reactions"] = [interactions.Reaction(**item) for item in data_dict["reactions"]]
    return data_dict
