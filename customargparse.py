...
class HMDMC_ArgumentParser(ArgumentParser):
    ...
    def add_default_args(self):
        ...
        self.add_argument(dest="system", type=str, help="Name of the system")
        self.add_argument(dest="prefix", type=str, help="Prefix of the system")
        ...
        self.add_argument(
            "-scaling_criteria_rollingmean_stddev",
            dest="scaling_criteria_rollingmean_stddev",
            default=0.1,
        ...
        self.add_argument(
            "-scaling_criteria_rollingmean_cycles",
            dest="scaling_criteria_rollingmean_cycles",
            default=3,
        ...
        self.add_argument(
            "-scaling_criteria_concentration_slope",
            dest="scaling_criteria_concentration_slope",
            default=0.1,
        ...
        self.add_argument(
            "-scaling_criteria_concentration_cycles",
            dest="scaling_criteria_concentration_cycles",
            default=1,
            help="Minimum (greater than or equal to) number of MDMC cycles that a species number fraction must be stagnant for reactions involving that species to be scaled.",
        )
        self.add_argument(
            "-scaling_criteria_rxnselection_count",
            dest="scaling_criteria_rxnselection_count",
            default=1,
        ...
        self.add_argument(
            "--log", dest="log", default=False, action="store_const", const=True
        )

    def read_notebook(self):
        notebook = pd.read_excel(
            self.args.filename_notebook, sheet_name=None, index_col=None, header=None
        )
        ...
        row_map = {
            key: {v[2]: v[0] for v in categories if v[1] == key}
    ...
    def adjust_default_args(self):
        ...
        self.args.number_of_voxels = [
            int(float(_)) for _ in self.args.number_of_voxels.split()
        ]
        ...
        self.args.scaling_criteria_rollingmean_stddev = float(
            self.args.scaling_criteria_rollingmean_stddev
        )
        self.args.scaling_criteria_rollingmean_cycles = int(
            float(self.args.scaling_criteria_rollingmean_cycles)
        )
        self.args.scaling_criteria_concentration_slope = float(
            self.args.scaling_criteria_concentration_slope
        )
        self.args.scaling_criteria_concentration_cycles = int(
            float(self.args.scaling_criteria_concentration_cycles)
        )
        self.args.scaling_criteria_rxnselection_count = int(
            float(self.args.scaling_criteria_rxnselection_count)
        )
        ...
        self.args.windowsize_scalingpause = int(
            float(self.args.windowsize_scalingpause)
        )
        self.args.windowsize_rxnselection = int(
            float(self.args.windowsize_rxnselection)
        )
        ...
        if len(self.args.__dict__) != num_starting_args:
            print(
                "Error! The number of arguments changed during argument adjustment."
