...
def main(argv):
    ...
    tfs = [
        os.path.exists(f)
        for fidx, f in enumerate(
            [
                args.filename_concentration,
                args.filename_scale,
                args.filename_log,
                args.filename_diffusion,
            ]
        )
    ...
    rxnscaling = pd.DataFrame(
        index=[1], columns=sorted(rxndata.keys()), data={_: 1 for _ in rxndata.keys()}
    )
    if args.scalerates:
    ...
    while Reacting:
        ...
        lines = [
            ...
            "ReactionScaling {}\n".format(
                " ".join(
                    [
                        "{}".format(rxnscaling.loc[list(rxnscaling.index)[-1], i])
                        for i in sorted(rxndata.keys())
                    ]
                )
            ),
        ]
    ...
    image_flags_x = np.where(
        atoms.x > box[0][1], np.ceil((atoms.x - box[0][1]) / box_length_x), 0
    )
    image_flags_x = np.where(
        atoms.x < box[0][0],
        np.floor((atoms.x - box[0][0]) / box_length_x),
        image_flags_x,
    )

    box_length_y = np.abs(box[1][1] - box[1][0])
    image_flags_y = np.where(
        atoms.y > box[1][1], np.ceil((atoms.y - box[1][1]) / box_length_y), 0
    )
    image_flags_y = np.where(
        atoms.y < box[1][0],
        np.floor((atoms.y - box[1][0]) / box_length_y),
        image_flags_y,
    )

    box_length_z = np.abs(box[2][1] - box[2][0])
    image_flags_z = np.where(
        atoms.z > box[2][1], np.ceil((atoms.z - box[2][1]) / box_length_z), 0
    )
    image_flags_z = np.where(
        atoms.z < box[2][0],
        np.floor((atoms.z - box[2][0]) / box_length_z),
        image_flags_z,
    )
    ...
    while overlaps is True:
        ...
        for idx_i, id_i in enumerate(atoms.ids):
            for idx_j, id_j in enumerate(atoms.ids[idx_i + 1 :], start=idx_i + 1):
                if not math.isclose(
                    atoms.x[idx_i], atoms.x[idx_j], rel_tol=0, abs_tol=overlap_tolerance
                ):
                    continue
                if not math.isclose(
                    atoms.y[idx_i], atoms.y[idx_j], rel_tol=0, abs_tol=overlap_tolerance
                ):
                    continue
                if not math.isclose(
                    atoms.z[idx_i], atoms.z[idx_j], rel_tol=0, abs_tol=overlap_tolerance
                ):
                    continue
        ...
        if len(repeats_wrapped) != 0:
            print(
                "Found overlapping wrapped atoms! Removing {} overlaps...".format(
                    len(repeats_wrapped)
                )
            )
        ...
        if len(repeats_unwrapped) != 0:
            print(
                "Found overlapping unwrapped atoms! Removing {} overlaps...".format(
                    len(repeats_unwrapped)
                )
            )
