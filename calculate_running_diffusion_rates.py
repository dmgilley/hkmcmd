...
def main(argv):
    ...
    parser.add_argument('-number_of_steps', dest='number_of_steps', default=10, type=int)
    ...
    print(f'{datetime.datetime.now()} -- Calculating direct voxel transition rates ({args.prefix})...')
    ...
    for species in args.diffusion_species:
        for replicate in np.arange(2):
            print(f'{datetime.datetime.now()} -- Performing random walk number {replicate} ({args.number_of_steps} steps) ({args.prefix})...')
            diffusion.perform_random_walks(number_of_steps=args.number_of_steps,species=species)
            ...
            with open(args.prefix+'.diffusion_rates_by_walks.txt','a') as f:
                ...
                f.write(f'{diffusion_rate[species].to_string(header=False,index=False)}\n')
